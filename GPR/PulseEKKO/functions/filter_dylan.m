function out = filter_dylan(a,dt,fmin,fmax,btord,verbose)
%
% USAGE:    a = filter_dylan(a,dt,fmin,fmax,btord,verbose)
%
% INPUT:
...
for ii = 1:ntrc
    tmp = a(:,ii);
    %         tmp = [tmp;zeros(numel(tmp),1)];
    tmp=a(:,ii)-mean(a(:,ii));
    %         tmp=demean(tmp);
    %         tmp=detrend(tmp);
    % tmp = medfilt1( tmp, MedFiltR, [], 2,'omitnan','truncate' );
    w = tukeywin(numel(tmp),0.05); % taper
    tmp=filtfilt(z,q,tmp.*w);
    out(:,ii) = tmp(1:nsamp);
    %         out(:,ii) = medfilt1( filtdat, MedFiltR, [], 2,'omitnan','truncate' );
end
% end
