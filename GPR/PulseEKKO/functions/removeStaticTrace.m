function [ removeTraces ] = removeStaticTrace( Rad, l2Rad, trhd, multiplexNtrcs, nearChan,  nChan )
...
[sortStaticTraces,sortIx] = sort(staticTraces);
...
% Check Algorithm Against Velocity
velIx = find(trhd(18,staticTraces) > .5);
% velIx = find(trhd(18,sortStaticTraces) > .5);
staticTraces(velIx) = [];
velIx = find(trhd(18,nearPlex) < .5);
rmIx = [staticTraces(:);velIx(:)];
staticTraces = unique(rmIx);
